...
app.get('/get-complex', (req, res) => { 
    ...
    var sql = `
            DELIMITER //
        CREATE PROCEDURE complexFoods(
        IN cat VARCHAR(250),
        IN cat2 VARCHAR(250),
        IN food1 VARCHAR(250),
        IN food2 VARCHAR(250),
        OUT total FLOAT)
        ...
        FROM Recipe r
        JOIN Food f ON r.Ingredients LIKE CONCAT('%', food1, '%') 
        ...
        SELECT r.RecipeTitle, r.Ingredients, r.Directions
        FROM Recipe r
        JOIN Food f ON r.Ingredients LIKE CONCAT('%', food2, '%') 
        WHERE f.Category = cat2
        );

        SELECT AVG(f.Protein) INTO total
        FROM Recipe r
        JOIN Food f ON r.Ingredients LIKE CONCAT('%', f.food1, '%') 
        GROUP BY Category
        HAVING f.Category = cat;
    ...
    const { cat, cat2, food1, food2 } = req.query; 
    
    const sql = 'CALL complexFoods(?, ?, ?, ?)'; 
    
    connection.query(sql, [cat, cat2, food1, food2], (error, results) => { 
    
        if (error) throw error; res.send(results[0]); 
    }); 
}); 
